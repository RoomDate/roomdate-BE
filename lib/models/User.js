...
module.exports = class User {
    ...
    static async createNewUser(username){
        ...
        if(rows.length === 0){
            ...
            const { rows } = await pool.query(`
           INSERT INTO users_main (username) VALUES ($1) RETURNING *
            `, [username]);
    ...
    static async insertNewUser({ username }){
        const { rows } = await pool.query(
    ...
    // static async insertNewUser({ username }){
    //     const { rows } = await pool.query(
    //         'INSERT INTO users_main (username) VALUES ($1) RETURNING *', [username]
    //     );
    ...
    static async sendLike(id, current_user){
        ...
        console.log('LENGTH', unique_key_exist.rows.length);
        

        if(unique_key_exist.rows.length === 0){

            console.log('HEEERREEEE 2');
            await pool.query(
